class ProjectsController < ApplicationController
  ...
  def destroy
	...
	def users
		...
		@other_users = @tenant.user.where(tenant_id: @tenant.id, is_admin: false) - (@project_users + [current_user])
	end
	...
	def add_user
		...
		respond_to do |format|
			if @project_user.save
				format.html {redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.tenant_id), notice: "User was successfully add to project"}
			else
				format.html { redirect_to users_tenant_project_url(id: 2project.id, tenant_id: @project.tenant_id), error: "User was not added to project" }
	end
  ...
  private
	...
	def verify_tenant
		unless params[:tenant_id] == Tenant.current_tenant_id.to_s
			redirect_to :root, flash: {error: 'You are nto authorized to access any organization other than your own'}
		end
	end
end
